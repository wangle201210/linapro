...
**Reason**: SQLite 不再是受支持运行模式，系统应直接拒绝 `sqlite:` 链接，而不是进入 SQLite 单机模式。

**Migration**: 使用 PostgreSQL 链接；单节点部署通过 `cluster.enabled=false` 表达。

### Requirement: SQLite 方言下集群专属组件必须跳过启动

**Reason**: SQLite 启动路径已移除，不再需要 SQLite 专属的集群组件跳过规则。

**Migration**: PostgreSQL 单节点模式继续通过 `cluster.enabled=false` 跳过集群组件。
