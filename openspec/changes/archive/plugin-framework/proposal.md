...
- **插件命名来源与市场安装**：去掉 `plugins.sources` 外壳与顶层独立 `marketplace`；在 `plugins.<自定义名>` 下声明命名来源，强制 `type` 为 Git（仓级 `repo`/`root`/`ref` + items id）或市场（`url` + items `id`/`version`，经 `distribution` 按 `mode=git|https` 落盘）；入口仍为 `plugins.install`/`plugins.update`，`v=` 仅覆盖市场版本；同一 plugin id 不得跨来源。
...
- `plugin-workspace-management`：插件工作区管理；命名来源（Git 仓级 ref / 市场 distribution）、安装更新、锁定与状态诊断。
...
- `plugin-workspace-management`：配置形状演进为命名来源（Git/市场）；安装与更新入口保持 `plugins.install`/`plugins.update`，市场经 distribution 落盘。
...
- 开发工具影响：`hack/config.yaml` 插件命名来源、`linactl` 安装/升级路径与 README 中英文；配置为破坏性结构调整，官方默认配置同步迁移。
- 「管理」入口无后端 API、数据模型或权限码变更；管理页判定与跳转全部在前端完成。
- 数据权限影响：租户与组织 scope 过滤只迁移类型归属和注入路径，过滤语义和拒绝策略不变；动态插件缓存、锁和存储仍按插件 ID 和租户上下文隔离；市场安装经 API + token，不绕过服务端过滤。
- 缓存一致性影响：Cache 继续复用启动期共享后端；统一 `publishPluginChange` 入口复用 `plugin-runtime` revision controller；命名来源工具缓存仅本地 `temp/plugin-sources`。
- i18n 影响：动态插件 i18n host service 移除属于边界调整；「管理」入口新增宿主前端 `pages.system.plugin.actions.manage` 等中英文文案与无管理页/未安装/路由不可访问提示；builtin 列表标识新增「内置插件」/ “Built-in” 及卸载禁用 tooltip 文案；命名来源变更无运行时 UI 文案义务。
