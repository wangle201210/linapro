...
登录成功后的落地页同样属于认证与会话边界：当管理员禁用工作台默认菜单（如 `/dashboard/analytics`）后，前端不得再无条件回退到写死的默认首页，而必须落到当前用户可访问菜单中的第一个可导航路径，避免登录即 404。

压缩后的归档将`user-auth`作为多租户、租户感知认证、租户成员关系、租户配置覆盖、租户数据隔离、会话热状态、多租户插件治理以及登录落地路径的历史 owner。组织、日志、通知、调度、插件运行时、工作台、数据库引导和`E2E`等能力只保留交叉影响摘要，当前契约以`openspec/specs/<capability>/spec.md`为准。
...
- 修复登录落地页：当工作台等默认菜单被禁用时，不得再无条件回退到写死的 `/dashboard/analytics`。落地路径优先级为显式可访问 `redirect` → 可访问 `homePath` → 侧栏第一个可导航菜单 → 安全兜底（如个人中心）；动态路由装配后对登录落地意图做可访问性纠偏，深链与真实 404 保持原语义。
...
- `user-auth`：登录、登出、租户选择、租户切换、token 撤销、impersonation 和认证事件全部携带租户与`clientType`上下文；`homePath`与登录落地路径必须从当前会话可访问且启用的菜单解析，默认工作台不可用时落到首个可导航菜单而非写死地址。
...
- 前端影响：登录页租户选择、工作台租户切换、平台租户管理、impersonation 提示、插件安装模式选择和 LifecycleGuard reason 展示；登录跳转、权限守卫、访问刷新与布局首页统一走可访问落地路径解析，`defaultHomePath` 仅作配置占位不得在目标不可访问时强制落地。
