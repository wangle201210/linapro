...
登录成功后的落地路径由后端 `homePath` 与前端可访问性纠偏共同决定，而不是写死工作台默认地址。

后端 `resolveHomePath` 从用户当前启用且可导航的内部菜单中选取适合作为落地页的稳定路由；工作台等默认菜单停用、隐藏或未授权时，必须落到菜单顺序中的下一个可导航内部路由，空树时可回退个人中心等安全兜底。候选应与侧栏可见可导航规则一致，排除外链、iframe 与非菜单类型。

前端统一 `resolvePostLoginLandingPath` 优先级：

1. 非空且当前会话可访问的显式 `redirect`
2. 非空且可访问的 `homePath`（经租户 `resolveFallbackPath` 约束后）
3. 侧栏深度优先遍历得到的第一个可导航叶子路径
4. `/profile` 或租户/平台约定的安全兜底

`preferences.app.defaultHomePath` 可保留为配置占位，但不得在目标路径不可访问时强制落地。可靠纠偏放在权限守卫 `generateAccess` 完成之后，覆盖密码登录、外部登录 handoff、已登录再进登录页与会话恢复；纠偏范围仅限登录落地意图（`redirect` 查询、`/`、写死 `defaultHomePath`），不得把任意深链或真实 404 改写为首个菜单，也不得破坏插件停用后的 `FallbackNotFound` 语义。
