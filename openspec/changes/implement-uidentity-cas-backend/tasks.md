...
- FB-1/FB-3 仍在推进：旧 admin 的 OAuth 授权码完整流程、账号导入检查/导入、短信发送、文件上传、健康检查、服务器监控、日志快照、作业/作业日志资源和 LDAP/外部文件/作业/外部监控动作边界已迁入插件；仍需继续做旧 admin 服务端接口兼容性总审计，尤其是实际 LDAP 同步执行器和外部执行器是否应由插件承载或委托宿主稳定能力。
