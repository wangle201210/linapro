...
- FB-1 仍在推进：旧 admin 的 OAuth 授权码完整流程、账号导入检查/导入、短信发送、文件上传、健康检查、服务器监控、日志快照、作业/作业日志资源、微信扫码登录状态链和 LDAP/外部文件/作业/外部监控动作边界已迁入插件；仍需继续做旧 admin 服务端接口兼容性总审计，尤其是确认默认系统管理、代码生成、页面重定向壳和第三方真实执行器哪些应明确列为非目标或外部边界。
