...
- [x] **FB-59**: 地图平移边界贴着底图边缘，位于边角的牛（如至善牛）无法被移到屏幕中心，纵向甚至完全不能拖动
- [x] **FB-60**: 地图牛只详情仍展示“查看 3D 牛”按钮，地图标记也保留`3D`角标，需要隐藏全部 3D 入口信号
- [x] **FB-61**: 喂草榜只有个人贡献排行，缺少按牛只累计有效喂草效果聚合的牛只排行
- [x] **FB-58**: 成都校区标定与初始牛只坐标偏离真实校区约`2.9km`，且比例尺把一个校区放大成`9.3km`见方
- [x] **FB-57**: 农场、图鉴、我的三个页面底部内容被固定导航栏遮挡且无法继续上滑
- [x] **FB-56**: 完善资料提醒条压在状态栏与胶囊下方无法点击，云搬牛地图保留了后端不存在的“等待出发”状态与“轨迹”工具
...
- `FB-56`根因：完善资料提醒条与`FB-55`的右列同源——`.profile-prompt`同样把`top`写死为`28px`，落在状态栏与胶囊按钮覆盖区内，「去完善」和关闭按钮被系统层压住而无法点击。云搬牛地图则残留了早期"轨迹会话"模型的产物：后端只有"每天最多`12`次位置上报"，没有开始、暂停、结束的会话概念，因此空闲态的"等待出发"徽标描述了一个不存在的状态；"轨迹"工具按钮同样来自该模型，仅用于重新框选折线，在没有会话语义后价值不足。

- `FB-57`根因：三个带底部导航的页面各自在滚动内容末尾放了`&__footer-space { height: 50px }`占位，但固定导航栏实际占据约`122`设计像素（自身高度约`98`加距底`24`），带`home indicator`的机型还要再加安全区。占位高度只有实际需要的四成，滚动到底后最后一行仍被导航压住。该常量在三个页面各写了一份，与导航真实高度没有任何关联，导航样式调整后必然继续脱节。
...
- `FB-56`补充影响：仅修改`cow-mini`首页提醒条定位、页面模型命名与云搬牛地图组件；不涉及后端、接口契约与其他页面。移除"轨迹"按钮的同时删除其仅有的调用目标`fitRoute`，避免留下死代码；"定位"工具保留。无数据权限、缓存、`DI`、接口性能、`i18n`与开发工具影响。

- `FB-57`补充影响：仅修改`cow-mini`底部导航组件样式与三个页面的占位引用；不涉及后端、接口契约与页面逻辑，无新增纯函数，改由编译产物核对与构建门禁验证。无数据权限、缓存、`DI`、接口性能、`i18n`与开发工具影响。
...
- `FB-56`修复：把`FB-55`的胶囊安全偏移从右列专用改为顶部悬浮元素通用（`rightColTop`更名为`capsuleSafeTop`），并按用户要求把首页全部顶部悬浮元素纳入同一治理：提醒条、资料校验失败提示条与左右两列共用避让偏移`capsuleSafeTop`；左上角校区定位条改用新增的`capsuleAlignTop`与胶囊同排对齐（它在胶囊左侧不冲突，但必须跟随胶囊纵向位置，否则落进状态栏）。样式表保留兜底值并注明来源。云搬牛地图在空闲态不再渲染状态徽标，"等待出发"文案随之移除，真机地图与`H5`预览两条渲染路径同步处理；"轨迹"工具按钮及其`fitRoute`处理函数一并删除。

- `FB-57`修复：把占位高度从三处页面常量收敛为导航组件自身维护的`.bottom-nav-space`，取值提高到`150`设计像素并叠加`safe-area-inset-bottom`（同时提供`constant()`与`env()`两种写法兼容旧基础库）；农场、图鉴、我的三页改用该共享占位并删除各自的旧常量。导航高度与配套占位自此在同一文件维护，后续调整导航不会再让页面底部失配。
...
- 已执行并通过（`FB-56`）：`home-page-model.test.ts`的胶囊偏移用例随更名同步更新并继续覆盖三类机型与五种取不到胶囊信息的情况；静态检索确认仓库内已无`等待出发`、`轨迹`工具与`fitRoute`残留，`定位`工具保留。静态排查确认只有首页声明`navigationStyle: custom`，其余页面使用微信原生标题栏、不存在胶囊重叠面；首页五处顶部悬浮元素（定位条、左列、右列、提醒条、校验失败提示）已全部按实测胶囊定位。审查中另发现`iron-panel`、`iron-faq`、`iron-cta`三组样式在`tsx`中已无任何引用，属`FB-33`将铁牛搬运界面迁往云搬牛分包后遗留的死样式，与本次遮挡问题无关，未在本轮删除。`cow-mini/miniapp`的`pnpm exec tsc --noEmit`通过，Vitest`17`个文件`124`个用例通过，`pnpm build:weapp`编译通过，`git diff --check`无问题。提醒条与胶囊的实际位置关系只能在微信真机或开发者工具中确认，需在真机上验证「去完善」与关闭按钮均可点击。
- 已执行并通过（`FB-57`）：静态排查确认仓库内引用底部导航的页面共三个（农场、图鉴、我的），均已改用共享占位且无`footer-space`残留；云搬牛为分包页面，不使用底部导航，无同类问题。核对`pnpm build:weapp`编译产物确认占位实际编译为`height:150rpx`并保留两条安全区回退声明，排除了`calc()`内像素单位未被转换而导致空白过大的风险。`pnpm exec tsc --noEmit`通过，Vitest`17`个文件`124`个用例通过。三页滚动到底后的实际留白需在真机确认，尤其是带`home indicator`的机型。
- 已执行并通过（`FB-56`扩展排查）：`home-page-model.test.ts`新增`capsuleAlignTop`的`2`组用例，覆盖三种胶囊纵向位置的取整对齐与四种取不到胶囊信息时的兜底；连同既有偏移用例，`cow-mini/miniapp`的 Vitest 为`17`个文件`126`个用例并全部通过，`pnpm exec tsc --noEmit`与`pnpm build:weapp`通过。弹层类容器一并复核：`iron-panel__sheet`为`max-height`加`overflow: auto`、云搬牛`FAQ`与消息中心均为`max-height`配内部`ScrollView`、证书弹层内图片另有`62vh`上限，均不会出现内容被裁切且无法滚动的情况。顶部与底部遮挡的真机表现仍需在微信客户端确认。
- `FB-58`根因：成都校区标定在三处各自硬编码且都不指向真实校区。后端`miniappconfig.go`的`campus(CampusChengdu, ..., 30.7058, 103.8318, 0.56, 16644, 8241)`是生产实际下发给小程序的权威值，`cow-mini`的`mockdb.ts`与《小程序接口文档》示例复制了同一组值。该原点距四川农业大学成都校区（温江区惠民路 211 号）真实位置约`2.9km`；比例尺`0.56 m/px`乘以`16644`像素等于`9.3km`幅宽，而底图实为该校区的斜视手绘图，图上校区图形仅`13523x6372`像素、对应地面约`920m x 430m`。此外`campus()`把仿射平移固定为图心`(8322,4120.5)`，而校区图形重心在`(5654,2931)`，即使原点正确也会整体错位。生产库牛只坐标与`manifest/sql/mock-data`完全一致，说明运营从未录入真实坐标，生产同样在用这组错值。
- `FB-58`坐标依据：真实中心取三个来源的聚合点。维基百科`30.70972,103.85972`与`Wikidata`的`30°42'25"N,103°51'38"E`均为`WGS-84`，按标准`WGS-84 -> GCJ-02`换算后分别为`30.70701,103.86183`与`30.70423,103.86267`（成都地区偏移约`-302m`纬、`+202m`经）；仓库内《方案B-自绘校园地图与经纬度映射规格》给出的原点为`30.7050,103.8650`。三者互距均在`350m`内，取聚合点`30.7054,103.8632`，与三个来源的距离均不超过`200m`。规范第`187`行要求接口坐标全链路使用`GCJ-02`，因此不得直接写入维基的`WGS-84`原始值。
- `FB-58`影响分析：修改文件为`backend/internal/service/miniappconfig/miniappconfig.go`、`miniappconfig_test.go`、`manifest/sql/mock-data/001-sicau-niu-mock-data.sql`、`小程序接口文档.md`，以及`cow-mini`的`src/lib/geo.ts`、`geo.test.ts`、`src/services/mockdb.ts`。受影响模块为小程序公开配置、校园地图渲染与`LBS`激活判距。后端新增`anchoredCampus`允许把原点钉到任意像素锚点，`campus`改为委托它并保持图心行为，都江堰与雅安校区标定不变。小程序侧对称新增`buildAnchoredCalibration`，`buildSimpleCalibration`委托它，`mockdb`的牛只仍按图上比例位置摆放，仅其反解出的经纬度随标定改正。`i18n`无影响：插件未启用`i18n`，未改动任何用户可见文案。缓存一致性无影响：公开配置快照的读写路径未改，仍由既有规则投影装配。数据权限无影响：未新增数据访问路径，未激活牛只仍只返回模糊区域。开发工具跨平台无影响：未改动脚本与构建工具。已读取`AGENTS.md`及命中的`.agents/rules/backend-go.md`、`.agents/rules/database.md`、`.agents/rules/frontend-ui.md`、`.agents/rules/testing.md`、`.agents/rules/documentation.md`、`.agents/rules/openspec.md`、`.agents/rules/i18n.md`；`sicau-niu`插件根目录不存在`AGENTS.md`。
- 已执行并通过（`FB-58`）：底图度量以像素实测为准，把`z0`层`5x3`分片拼回`2560x1536`后按非背景像素求得校区图形包围盒`x[0,2080] y[48,1028]`、重心`(869.7,450.8)`，折算回`16644x8241`原图为包围盒`x[0,13523] y[312,6684]`、重心`(5654,2931)`，据此确定锚点并按校区地面幅宽约`920m`反算比例尺`0.068 m/px`。初始牛只与三头铁牛地标按原有相对布局整体重排到新中心，跨度收敛为南北`300m`、东西`660m`，逐点投影校验全部落在校区图形包围盒内。后端新增`TestChengduCalibrationMatchesSeededCattle`，断言下发的成都校区原点不得偏离真实校区超过`0.005`度，并把跨度最大的两头种子牛投影回底图确认仍在图形范围内；以`GOWORK=off`运行`go build ./...`与`go test ./backend/...`全部通过。`make lint dir=apps/lina-plugins/sicau-niu plugins=1`为`0 issues`（根`make lint`失败的两项`cyclop`与`revive`均位于`apps/lina-plugins/cms`，属既有问题，与本次无关）。`cow-mini/miniapp`的`pnpm exec tsc --noEmit`通过，Vitest`17`个文件`130`个用例通过，`geo.test.ts`新增`4`个用例覆盖原点落在指定锚点而非图心、比例尺按米/像素生效且正北朝上、锚点取图心时与`buildSimpleCalibration`完全等价、以及种子牛只坐标落在底图校区图形范围内。《小程序接口文档》的标定示例已同步为新原点与`a=14.705882`、`tx=5654`、`ty=2931`。本次未新增 SQL 迁移与`DAO`生成变化，未运行`make db.init`与`make dao`。
- `FB-58`遗留边界：底图是斜视手绘图而非正射影像，单一仿射只能整体近似，无法保证「信工守护牛」正好落在信息工程学院楼下；三个坐标来源之间仍有约`350m`离散，聚合点属工程取值而非实测。彻底解决需按《方案B》第`4`节 SOP 现场采集`6`个地标控制点（`4`解`2`验）后用`scripts/calibrate.ts`求解，本轮不具备现场条件。另需注意本修复只改变随插件分发的初始数据与默认标定，**不会自动改写生产库中已存在的牛只坐标**；生产上线前必须由运营在管理端重新录入或执行一次数据订正，否则线上仍是旧坐标。
- `FB-59`根因：`CampusMap`把`movable-view`的内容尺寸直接设为底图显示尺寸`dispW x dispH`，而`movable-view`不能拖出`movable-area`，可平移区间恒为`[viewport - content * scale, 0]`。底图按`cover`适配（`base = max(viewport.w / imageW, viewport.h / imageH)`），成都校区底图宽高比`16644:8241`远比竖屏视口扁，因此适配后`dispH`恰好等于视口高度，纵向可平移距离精确为`0`——垂直方向根本拖不动；横向也只有`dispW - viewport.w`的行程，底图边缘最多停在视口边缘。结果是位于底图边角的牛（如`NIU-010`至善牛）永远无法被移到屏幕中心，且`centerTo`的夹取会把轮播联动居中请求默默截断。以`390x844`实测：改前内容盒`1704.59 x 844`，纵向行程`0px`。
- `FB-59`影响分析：修改文件为`cow-mini`的`src/components/CampusMap/index.tsx`、`index.scss`，新增`campus-map-model.ts`与`campus-map-model.test.ts`。受影响模块仅小程序首页校园地图的平移与居中交互。方案为在底图四周各留半个视口的透明留白，内容盒变为`(dispW + viewport.w) x (dispH + viewport.h)`，底图整体偏移到留白内，覆盖物经`toDisp`加同一偏移，两者仍严格对齐；留白区域露出`.campus-map`既有底色`#cfe8c4`，与底图外围草地同色，观感上不突兀。`TileLayer`只用`x/y`做细节层瓦片裁剪，按`livePos + pad * viewScale`传入保持裁剪口径一致；基础层始终全量渲染，即使裁剪有偏差也只会多加载细节瓦片，不会造成错位。数学证明留白比例`0.5`是充分的：底图右下角居中所需位移为`viewport/2 - (pad + disp) * s`，合法下界为`viewport - content * s`，两者之差为`viewport * (s - 1) / 2`，在`scale >= 1`时恒非负，故`scaleMin=1`到`scaleMax=2.5`全区间任意底图点都能居中。`i18n`无影响：未改动任何用户可见文案。缓存一致性、数据权限、数据库、开发工具跨平台均无影响：纯前端布局计算，未触及数据访问或脚本。已读取`AGENTS.md`及命中的`.agents/rules/frontend-ui.md`、`.agents/rules/testing.md`、`.agents/rules/openspec.md`、`.agents/rules/i18n.md`；本次不涉及插件目录，`sicau-niu`插件根目录`AGENTS.md`仍不存在。
- 已执行并通过（`FB-59`）：新增`campus-map-model.test.ts`共`6`个用例，覆盖四周留白尺寸换算、底图左上角与右下角均可居中（左边缘居中时`movable-view`恰停在正向极限`0`、右边缘恰为`-dispW`）、`1/1.5/2/2.5`四档缩放下底图五个横纵分位点全部可居中、结果始终落在`movable-view`合法平移区间内，以及一条回归用例断言旧的「内容盒即底图」边界下边角点`canCenter`为`false`而中点为`true`，把缺陷本身锁进测试。`cow-mini/miniapp`的`pnpm exec tsc --noEmit`通过，Vitest`18`个文件`136`个用例通过，`pnpm build:weapp`编译通过（仅剩既有`cow.glb`体积与异步分包提示）。以`390x844`视口在浏览器加载`H5`开发构建做渲染核验：瓦片底图与牛`Pin`位置对齐正常，加留白未破坏图层定位；`DOM`实测内容盒为`2094.59 x 1688`，即高度恰为视口的两倍、纵向行程从`0px`变为`844px`，与几何推导一致。真实拖拽未能在`H5`中驱动——`Taro`的`movable-view`为`Web`兼容实现，合成`touch`事件未触发其位移，且其边界行为与微信原生组件本就可能不同，因此拖拽手感与边角牛实际可居中仍需在微信真机或开发者工具确认，属本轮验证边界，不以`H5`结果代替。该页面为小程序运行时，沿用既有等价验证边界，不新增插件`TC`编号。
- `FB-60`根因与修复：小程序首页详情弹层直接渲染“查看 3D 牛”按钮并导航到 3D 分包，地图已激活牛标记同时固定渲染`3D`角标。现已移除这两处可见入口和对应导航函数，保留独立 3D 页面文件本身，不影响详情、校史卡片、喂养统计和关闭弹层。
- `FB-61`根因与修复：现有`GET /player/rankings/feed`只按玩家聚合喂草效果，响应和小程序状态均只有个人榜。接口现增加`niuList`，在数据库侧按`niu_id`执行`SUM(effect_amount)`、稳定排序和`TopN`限制，再用一次`WHERE IN`投影批量装配牛只编号与名称；个人榜与牛只榜在同一请求只读取一次运行时`TopN`快照。小程序真实适配器和开发 Mock 均映射为`personal/cattle`两组数据，个人中心使用分段控件切换“个人贡献”和“牛只排行”。
- 影响判断（`FB-60`~`FB-61`）：修改范围闭环在`sicau-niu`插件玩家榜单契约、服务与文档，以及独立`cow-mini`页面和适配器；不修改`lina-core`、Vben、SQL、DAO、缓存、运行期依赖、构建脚本或开发工具。数据权限沿用已有玩家 token，新增牛只榜只暴露活动中原本可见的牛只 ID、编号、名称和聚合效果。接口性能为数据库分组加`TopN`和一次有界批量标签查询，不随返回行数产生`N+1`。`sicau-niu`未启用插件`i18n`，`cow-mini`为单语言中文，本轮不新增语言资源；API 文档源与机器契约已同步。无 DI、缓存一致性、数据库迁移或跨平台工具影响。
- 已执行并通过（`FB-60`~`FB-61`）：以 Go`1.26.4`、`GOWORK=off`和低并发对真实 PostgreSQL 运行 ranking 服务包全部测试，新增用例覆盖牛只累计效果聚合、同分竞赛排名`1/1/3`、稳定牛只 ID 顺序与批量名称/编号装配；玩家 Controller 和 API 包编译通过。目标插件`make lint dir=apps/lina-plugins/sicau-niu/backend/internal/service/ranking plugins=1`执行`golangci-lint`与`staticcheck`均为`0 issues`。`cow-mini/miniapp`的 TypeScript 检查通过，Vitest`18`个文件`137`个用例通过，微信与 H5 生产构建均成功，仅保留既有 bundle 和`cow.glb`体积警告。OpenAPI JSON 通过`jq`解析与目标`niuList/NiuRankItem`断言，三仓`git diff --check`和 OpenSpec 严格校验通过。
- E2E 与视觉判断（`FB-60`~`FB-61`）：`cow-mini`是独立微信小程序，不新增插件工作台`TC`编号；使用生产 H5 产物并在 Playwright 层拦截玩家 API 做等价页面验证。`390x844`视口下确认“个人贡献/牛只排行”可点击切换，牛名、编号、名次和累计点数无重叠或溢出；地图牛只标记无`3D`角标，详情弹层只保留“知道了”，校史卡片与统计完整。截图位于`cow-mini/temp/20260818/120230-profile-cattle-rank.png`和`120330-home-cow-detail-no-3d.png`并已视觉检查。微信原生组件的最终交互仍以真机为验收边界。
- `lina-review`结论（`FB-60`~`FB-61`）：反馈级审查已展开主仓、插件子仓和`cow-mini`的已跟踪与未跟踪增量，并重新读取`AGENTS.md`、OpenSpec、文档、架构、数据权限、插件、API 契约、后端 Go、前端 UI、测试和`i18n`规则；`sicau-niu`与`cow-mini`均不存在本地`AGENTS.md`。未发现阻塞问题或警告；后端聚合、前端映射、机器契约和可观察页面验证相互一致。根仓既有`go.work`、`cow-mini/.DS_Store`及此前`.playwright-cli`工具产物不属于本反馈，未修改或纳入结论。
