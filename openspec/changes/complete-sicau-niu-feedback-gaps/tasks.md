...
- `FB-33`根因：此前根据“铁牛搬运”名称把玩法实现为队长创建并结束全队共享会话、成员持续提交轨迹心跳的模型，没有在实现前确认“团关系”和“个人贡献”的产品语义。新规则取消距离组团、最低人数、队长控制、开始/结束和全队共享轨迹，改为同一时间唯一有效团绑定，以及每名成员基于相邻两次成功上报点独立累计直线位移贡献。规则继续收敛时又发现“贡献明细”没有明确覆盖首次基准点和零距离上报，存在只保留累计值而丢失正常上报统计事实的实现空间。原始表结构、接口和小程序交互均无法直接满足该规则。
...
- `FB-33`修复：移除玩家端离队、开始、心跳、结束共享会话 API 和旧会话/轨迹模型，新增固定团关系、团详情/分页成员、个人位置贡献上报与本人事实查询；服务使用相邻成功点的 Haversine 直线距离，首次及零距离成功上报均落库，按北京时间自然日限制每人最多`12`次并稳定幂等重放。有效团并发上限为`120`，用户同一时间唯一有效团；主节点定时任务和请求事务兜底将连续`72`小时无成功创建、加入或上报的团失效并关闭成员绑定。管理端新增团列表、改名、统计和坐标审计页面/API；小程序真实适配器、持久化 mock、首页团面板和契约快照统一切换到创建、加入、成员和上报贡献模型，旧玩法说明和未暴露的旧轨迹 mock/type 同步删除。最终审查补齐开发 mock 重启后本人成员明细自愈与调试重置，并让后端拒绝`NaN/Inf`坐标、钳制 Haversine 中间值以覆盖极端合法坐标。
