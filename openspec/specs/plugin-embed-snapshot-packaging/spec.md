...
系统 SHALL 允许动态插件像源码插件一样，通过显式的`go:embed`资源声明文件暴露`plugin.yaml`、`frontend`、`manifest`等资源集合，作为作者侧资源打包入口。`manifest`集合 SHALL 保留`manifest/`下所有实际文件的源码路径语义，包括`manifest/config/`、`manifest/sql/`、`manifest/i18n/`、插件实际提供的`manifest/metadata.yaml`和其他自定义资源。`metadata.yaml` SHALL 只是普通可选资源，不得作为单独 Metadata 服务或必填资源。

#### Scenario: 动态插件声明嵌入资源

- **WHEN** 一个动态插件需要把清单、前端静态资源、安装 SQL、运行期配置模板、i18n 资源或其他 manifest 资源随`.wasm`一起交付
...
#### Scenario: 动态插件嵌入 manifest SQL 和 i18n

- **WHEN** 动态插件通过`go:embed`声明`manifest/**`
- **AND** 插件源码包含`manifest/sql/001-schema.sql`和`manifest/i18n/zh-CN/plugin.json`
- **THEN** 构建器在资源集合中保留这些文件的相对路径
- **AND** 不要求插件为这些资源维护额外 metadata 文件
...
- **THEN** 构建器必须从该嵌入文件系统中读取`plugin.yaml`、前端资源、SQL 资源、`manifest/config/`资源、`manifest/i18n/`资源和其他通用 manifest 资源
- **AND** 构建器必须继续生成宿主已识别的 manifest、前端资源、SQL、配置、i18n 和通用 manifest 资源自定义节
- **AND** 生成后的`.wasm`产物仍可被宿主现有动态插件治理链路直接消费

#### Scenario: 快照区段保留 metadata 普通资源

- **WHEN** 动态插件源码包含`manifest/metadata.yaml`
- **THEN** 构建器生成的宿主快照区段保留该文件内容和相对路径
- **AND** 宿主运行时可通过动态插件资源视图向`Manifest()`提供该资源
- **AND** 构建器不生成独立 Metadata 区段

#### Scenario: 快照区段保留非 YAML manifest 资源

- **WHEN** 动态插件源码包含`manifest/i18n/zh-CN/plugin.json`或`manifest/sql/001-schema.sql`
- **THEN** 构建器生成的宿主快照区段保留文件内容和相对路径
- **AND** 不因为资源不是 YAML 而排除出通用 manifest 资源视图

### Requirement: 动态插件构建必须保留目录扫描兼容回退

系统 SHALL 在构建入口未声明`go:embed`资源集合时允许动态插件通过目录扫描方式完成构建。目录扫描回退 MUST 使用与`go:embed`路径一致的资源语义，包含`plugin.yaml`、前端资源、SQL 资源、`manifest/config/`资源、`manifest/i18n/`资源和其他通用 manifest 资源。本项目不保留旧 Metadata 服务兼容层，目录扫描回退只兼容资源来源方式，不兼容旧读取接口。

#### Scenario: 旧动态插件未声明嵌入资源

- **WHEN** 构建器处理一个尚未新增`go:embed`资源声明的动态插件
- **THEN** 构建器仍可按现有目录扫描方式读取`plugin.yaml`、前端资源、SQL 资源、`manifest/config/`资源、`manifest/i18n/`资源和其他通用 manifest 资源
- **AND** 宿主侧无需因为作者侧资源声明方式不同而修改运行时治理逻辑
...
#### Scenario: 目录扫描保留 manifest i18n 和 SQL 路径

- **WHEN** 目录扫描回退发现`manifest/i18n/zh-CN/plugin.json`和`manifest/sql/001-schema.sql`
- **THEN** 构建产物中的资源路径仍为原始路径
- **AND** 不生成独立 Metadata 路径或额外清单字段
...
动态插件发布产物 SHALL 保留与源码插件一致的插件配置和 manifest 资源路径语义。构建工具不得为`manifest/config/`、`manifest/sql/`、`manifest/i18n/`、插件实际提供的`metadata.yaml`或其他 manifest 资源引入不同于源码插件的额外路径约定或额外清单字段，除非先通过 OpenSpec 变更明确设计。
...
#### Scenario: 发布产物携带 metadata 普通资源

- **WHEN** 动态插件源码包含`manifest/metadata.yaml`
- **THEN** 发布产物携带该资源
- **AND** 宿主运行时可通过当前插件资源视图向`Manifest()`读取提供该文件
- **AND** 发布产物不包含独立 Metadata 服务声明

#### Scenario: 发布产物携带 SQL 和 i18n 原文资源

- **WHEN** 动态插件源码包含`manifest/sql/001-schema.sql`和`manifest/i18n/zh-CN/plugin.json`
- **THEN** 发布产物携带这些资源及原始路径
- **AND** 宿主运行时可在专用管线中消费它们
- **AND** 已授权插件可通过`Manifest()`读取它们的原文
