# config-param-value-types Specification

## Purpose
TBD - created by archiving change config-param-value-types. Update Purpose after archive.
## Requirements
...
系统 SHALL 在 `sys_config.options` 中以 JSON 数组持久化选项，元素包含 `label` 与 `value` 字符串字段。当 `value_type` 为 `select`、`radio` 或 `multi_select` 时，options MUST 为至少包含一个元素的合法 JSON 数组；其他类型可将 options 存为空串或空数组。
