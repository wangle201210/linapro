# plugin-lifecycle-global-hooks Specification
...
系统 SHALL 提供 `GlobalBeforeDisable` 与 `GlobalBeforeUninstall` 的注册与编排能力，供 owner 插件在目标插件禁用或卸载前表达依赖保护。一期邮件实现可选择性使用；未注册则无行为变化。
