...
- **THEN** 后端启动、数据库初始化或 mock 加载在方言解析阶段失败
- **AND** 错误消息说明 SQLite 不再支持，并列出当前支持的方言仅为 `pgsql:`
- **AND** 系统不注册 SQLite 驱动、不创建 SQLite 文件、不执行 SQLite DDL 转译
...
- **AND** 使用 `JSONB`、`CREATE TRIGGER`、`CREATE FUNCTION`、`MERGE`、`WITH RECURSIVE` 等 PG 高级特性前必须新立 OpenSpec 变更评估可维护性、升级策略、DAO 兼容性和测试覆盖
- **AND** MUST NOT 包含 MySQL 或 SQLite 特有语法（AUTO_INCREMENT、UNSIGNED、ENGINE=、INSERT IGNORE、ON DUPLICATE KEY UPDATE、COLLATE NOCASE 等）
