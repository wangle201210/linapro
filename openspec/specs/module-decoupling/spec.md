...
系统 SHALL 要求官方源码插件在各自插件根目录维护独立的 GoFrame ORM 代码生成配置，并通过插件本地的`dao/do/entity`完成数据库访问，避免重新依赖宿主`dao/model`包或长期保留散落的`g.DB().Model(...)`直连实现。插件代码生成配置 MUST 位于插件根`hack/config.yaml`；GoFrame 生成工作目录 MUST 保持为插件`backend/`目录。
