...
宿主 SHALL 提供基于配置文件的集群部署模式开关。系统必须支持 `cluster.enabled` 作为总开关，并支持 `cluster.election.lease`、`cluster.election.renewInterval` 作为选举子配置。未显式配置时，`cluster.enabled` 必须默认为 `false`。系统仅支持 PostgreSQL 方言（`database.default.link` 以 `pgsql:` 开头）作为运行时数据库；`sqlite:` 链接必须在方言解析阶段失败，不得进入集群配置覆盖或协调组件初始化流程。当数据库链接为 PostgreSQL 方言时，`cluster.enabled` 按用户配置生效，可启用集群模式。
...
- **THEN** 宿主在方言解析阶段返回 SQLite 不再支持的明确错误
- **AND** 系统不进入 cluster.enabled 覆盖逻辑
