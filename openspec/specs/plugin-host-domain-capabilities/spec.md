...
系统 SHALL 要求动态插件在`plugin.yaml hostServices`中声明领域`service + method`，并在安装或启用阶段由宿主确认授权后形成运行时授权快照。运行时调用 MUST 校验授权快照中的领域服务、方法和资源或投影范围。安装授权替代插件级菜单/RBAC 方法校验，但不得替代领域数据权限、租户边界、状态机、数量上限和审计校验。

#### Scenario: 动态插件调用已授权领域读取方法

- **WHEN** 动态插件声明并获得授权调用`service: user`和`method: users.batch_get`
