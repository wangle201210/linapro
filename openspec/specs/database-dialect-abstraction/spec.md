...
定义宿主数据库方言抽象层，用于统一收敛当前唯一支持的 PostgreSQL 运行时数据库能力，并为 SQL 入口、数据库准备、启动期钩子、错误分类、数据库版本查询和表元数据查询提供稳定的公共接缝。

## Requirements
### Requirement: 宿主必须通过统一的方言抽象层收敛数据库引擎差异

系统 SHALL 在 `apps/lina-core/pkg/dialect/` 提供公共稳定的 `Dialect` 接口与方言辅助能力作为数据库引擎相关行为的唯一收敛点。当前唯一支持的具体数据库方言 SHALL 为 PostgreSQL。所有数据库引擎相关的差异化行为（DDL 入口、数据库准备、集群能力查询、启动期钩子、驱动错误分类、数据库版本查询、表元数据查询）必须通过该包暴露，业务模块（`controller` / `service` / `model` / `dao`）不得在自身代码路径中出现 `if isPostgres / if isSQLite` 等数据库引擎判断。`pkg/dialect` 的公开签名 SHALL 只依赖稳定窄接口，不得暴露宿主 `internal` 包中的具体服务类型，也不得导出 PostgreSQL 方言具体实现类型；具体 PostgreSQL 实现应收敛在 `pkg/dialect/internal/postgres` 内部子包中，由公共工厂与公共门面能力统一委托。

#### Scenario: 业务模块不感知数据库引擎差异
- **当** 业务模块（如 `user` / `role` / `dict` / `kvcache` / `locker`）通过 DAO 层执行查询、写入、更新、删除操作时
- **则** 业务代码不包含针对数据库引擎的分支判断
- **且** 同一份业务代码在 PostgreSQL 方言下直接执行

#### Scenario: 所有方言相关行为通过 Dialect 接口暴露
- **当** 宿主需要执行"DDL 入口 / 数据库准备 / 集群能力查询 / 启动期钩子 / 数据库版本查询 / 表元数据查询"中的任一行为时
...
- **且** PostgreSQL 的 DDL 入口、数据库准备、启动期行为、驱动错误分类、表元数据查询实现维护在 `pkg/dialect/internal/postgres` 内部子包中
...
- **且** `pkg/dialect` 使用驱动暴露的结构化 SQLSTATE 进行分类，错误文案匹配最多只能作为方言包内部的显式兜底
...
- **且** 不返回错误（除非输入本身为显式无效输入）
- **且** `sourceName` 不影响 PostgreSQL no-op 转译结果

### Requirement: 方言必须暴露数据库准备入口

`Dialect.PrepareDatabase(ctx, link, rebuild)` SHALL 负责在执行 DDL 资源前完成方言相关的数据库准备工作。PostgreSQL 方言通过连接系统库 `postgres` 执行 `pg_terminate_backend` + `DROP DATABASE IF EXISTS` + `CREATE DATABASE`。`sqlite:`、`mysql:` 或未知链接 MUST 在方言解析阶段失败，不得创建本地数据库文件或进入 SQL 执行。PostgreSQL 目标数据库创建后，后续宿主 init SQL SHALL 直接创建业务表、索引、注释并写入 seed 数据，不创建自定义排序规则。
...
#### Scenario: SQLite 链接不准备数据库文件
- **当** 配置文件 `database.default.link` 以 `sqlite:` 开头且调用方尝试解析方言或执行 init
- **则** 系统返回 SQLite 不再支持的明确错误
- **且** 不创建 SQLite 父目录、数据库文件、WAL 文件或 SHM 文件
- **且** 不执行任何 DDL 转译
...
`Dialect.OnStartup(ctx, runtime)` SHALL 在宿主启动 bootstrap 阶段被调用一次。`runtime` SHALL 是 `pkg/dialect` 中定义的稳定窄接口。当前唯一支持的 PostgreSQL 方言为 no-op。`sqlite:` 链接 MUST 在方言解析阶段失败，不得进入启动期钩子或单机降级路径。
...
#### Scenario: SQLite 链接不会进入启动期钩子
- **当** 配置文件 `database.default.link` 以 `sqlite:` 开头时
- **则** `dialect.From(link)` 在创建方言实例前返回错误
- **且** 系统不调用 `OnStartup`
- **且** 系统不输出 SQLite 单机模式警告或修改 `cluster.enabled`

### Requirement: Dialect 接口必须提供表元数据查询能力

系统 SHALL 在 `Dialect` 接口中提供 `QueryTableMetadata(ctx context.Context, db gdb.DB, schema string, tableNames []string) ([]TableMeta, error)` 方法，用于查询数据表名与表注释。`TableMeta` SHALL 至少包含 `TableName string` 与 `TableComment string` 字段。调用方（如 plugin data service）SHALL 通过该方法查询表元数据，不得直接编写方言特有的 SQL（`information_schema.TABLES` / `pg_catalog.pg_description`）。
