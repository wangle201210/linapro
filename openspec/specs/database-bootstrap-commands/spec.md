...
系统 SHALL 将 `init` 和 `mock` 使用的每个 SQL 文件解析为独立语句的有序列表并逐个执行，而非依赖数据库连接字符串中的驱动级多语句支持。该规则适用于当前唯一支持的 PostgreSQL 方言：SQL 源文本由现有 `splitSQLStatements` 切分，再逐句通过 GoFrame `gdb` 提交。
...
系统 SHALL 在 `init` 命令执行 SQL 资源前，根据 `database.default.link` 协议头分发到对应方言的 `PrepareDatabase`。当前唯一支持的 PostgreSQL 方言通过连接系统库 `postgres` 执行 `pg_terminate_backend` + `DROP DATABASE IF EXISTS` + `CREATE DATABASE`。`sqlite:`、`mysql:` 或未知链接 MUST 在方言解析阶段失败，不得创建本地数据库文件或进入 SQL 执行。`mock` 命令 SHALL 依赖已由 `init` 初始化完成的目标数据库，不得创建、重建或准备数据库。引导命令实现不得直接编写 PostgreSQL 专属的链接解析、`pg_terminate_backend` 或 `DROP/CREATE DATABASE` 逻辑。
...
系统 SHALL 在 `init` / `mock` 执行每个 SQL 文件前，先调用当前方言的 `TranslateDDL(ctx, sourceName, ddl)` 入口处理单一 PostgreSQL 14+ 方言来源的 SQL 内容。`sourceName` SHALL 使用源 SQL 文件路径或嵌入资产路径。当前唯一支持的 PostgreSQL 方言下该入口为 no-op，返回原始 SQL。SQL 文件的源文件保持单一 PostgreSQL 14+ 方言来源。
