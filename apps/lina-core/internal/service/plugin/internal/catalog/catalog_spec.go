...
func ValidateResourceSpec(pluginID string, spec *ResourceSpec, filePath string) error {
	...
	if spec.Type == "" {
		spec.Type = plugintypes.ResourceSpecTypeTableList.String()
	}
	if plugintypes.NormalizeResourceSpecType(spec.Type) != plugintypes.ResourceSpecTypeTableList {
	...
	for _, filter := range spec.Filters {
		...
		if plugintypes.NormalizeResourceFilterOperator(filter.Operator) == "" {
	...
	if spec.OrderBy.Direction == "" {
		spec.OrderBy.Direction = plugintypes.ResourceOrderDirectionASC.String()
	}
	if plugintypes.NormalizeResourceOrderDirection(spec.OrderBy.Direction) == "" {
	...
	if len(spec.Operations) == 0 {
		spec.Operations = []string{plugintypes.ResourceOperationQuery.String()}
	}
	operationSeen := make(map[string]struct{}, len(spec.Operations))
	for _, operation := range spec.Operations {
		normalizedOperation := plugintypes.NormalizeResourceOperation(operation)
	...
	if _, needsKeyField := operationSeen[plugintypes.ResourceOperationGet.String()]; needsKeyField && spec.KeyField == "" {
		return gerror.Newf("plugin resource get operation requires keyField: %s", filePath)
	}
	if _, needsKeyField := operationSeen[plugintypes.ResourceOperationUpdate.String()]; needsKeyField && spec.KeyField == "" {
		return gerror.Newf("plugin resource update operation requires keyField: %s", filePath)
	}
	if _, needsKeyField := operationSeen[plugintypes.ResourceOperationDelete.String()]; needsKeyField && spec.KeyField == "" {
	...
	if _, needsWritableFields := operationSeen[plugintypes.ResourceOperationCreate.String()]; needsWritableFields && len(spec.WritableFields) == 0 {
		return gerror.Newf("plugin resource create operation requires writableFields: %s", filePath)
	}
	if _, needsWritableFields := operationSeen[plugintypes.ResourceOperationUpdate.String()]; needsWritableFields && len(spec.WritableFields) == 0 {
		return gerror.Newf("plugin resource update operation requires writableFields: %s", filePath)
	}

	if spec.Access == "" {
		spec.Access = plugintypes.ResourceAccessModeRequest.String()
	}
	if plugintypes.NormalizeResourceAccessMode(spec.Access) == "" {
