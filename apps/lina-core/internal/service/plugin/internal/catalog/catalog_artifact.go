// This file reads and validates dynamic plugin WASM artifact manifests without
// depending on runtime orchestration services.
...
func ParseRuntimeWasmArtifactContent(filePath string, content []byte) (*ArtifactSpec, error) {
	...
	embeddedManifest := &ArtifactManifest{}
...
func publicAssetDeclarationsMatch(left []*PublicAssetSpec, right []*PublicAssetSpec) bool {
...
func parseRuntimeArtifactSQLAssets(
	...
	sections map[string][]byte,
	sectionName string,
) ([]*ArtifactSQLAsset, error) {
	sectionContent, ok := sections[sectionName]
	if !ok {
		return []*ArtifactSQLAsset{}, nil
	}

	assets := make([]*ArtifactSQLAsset, 0)
	if err := json.Unmarshal(sectionContent, &assets); err != nil {
...
func parseRuntimeArtifactManifestResources(
	filePath string,
	sections map[string][]byte,
) ([]*ArtifactManifestResource, error) {
	sectionContent, ok := sections[bridgeartifact.WasmSectionManifestResources]
	if !ok {
		return []*ArtifactManifestResource{}, nil
	}

	assets := make([]*ArtifactManifestResource, 0)
	if err := json.Unmarshal(sectionContent, &assets); err != nil {
...
func parseRuntimeArtifactHookSpecs(
	...
	items := make([]*HookSpec, 0)
	if err := json.Unmarshal(content, &items); err != nil {
		return nil, gerror.Wrapf(err, "Failed to parse dynamic plugin backend hook contracts: %s", filePath)
	}
	for _, item := range items {
		if err := ValidateHookSpec(pluginID, item, filePath); err != nil {
			return nil, err
		}
	}
	return CloneHookSpecs(items), nil
}
...
func parseRuntimeArtifactResourceSpecs(
	...
	items := make([]*ResourceSpec, 0)
	if err := json.Unmarshal(content, &items); err != nil {
		return nil, gerror.Wrapf(err, "Failed to parse dynamic plugin backend resource contracts: %s", filePath)
	}
	cloned := make([]*ResourceSpec, 0, len(items))
	for _, item := range items {
		if err := ValidateResourceSpec(pluginID, item, filePath); err != nil {
			return nil, err
		}
		cloned = append(cloned, CloneResourceSpec(item))
	}
	return cloned, nil
}
...
func parseRuntimeArtifactFrontendAssets(
	...
	sections map[string][]byte,
	sectionName string,
) ([]*ArtifactFrontendAsset, error) {
	content, ok := sections[sectionName]
	if !ok {
		return []*ArtifactFrontendAsset{}, nil
	}

	assets := make([]*ArtifactFrontendAsset, 0)
