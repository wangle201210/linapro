...
func buildPluginResourceRefEntity(
	...
	pluginID string,
	releaseID int,
	descriptor *plugintypes.ResourceRefDescriptor,
	data do.SysPluginResourceRef,
...
func (s *serviceImpl) buildPluginResourceRefDescriptors(manifest *catalog.Manifest) []*plugintypes.ResourceRefDescriptor {
	if manifest == nil {
	...
	descriptors := []*plugintypes.ResourceRefDescriptor{
		newResourceRefDescriptor(
			plugintypes.ResourceKindManifest,
			pluginResourceKeyManifest,
			plugintypes.ResourceOwnerTypeFile,
	...
	if installSQLCount > 0 {
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindInstallSQL,
			pluginResourceKeyInstallSQLBundle,
			plugintypes.ResourceOwnerTypeInstallSQL,
	...
	if uninstallSQLCount > 0 {
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindUninstallSQL,
			pluginResourceKeyUninstallSQLBundle,
			plugintypes.ResourceOwnerTypeUninstallSQL,
	...
	if mockSQLCount > 0 {
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindMockSQL,
			pluginResourceKeyMockSQLBundle,
			plugintypes.ResourceOwnerTypeMockSQL,
	...
	if len(frontendPagePaths) > 0 {
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindFrontendPage,
			pluginResourceKeyFrontendPages,
			plugintypes.ResourceOwnerTypeFrontendPageEntry,
	...
	if len(frontendSlotPaths) > 0 {
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindFrontendSlot,
			pluginResourceKeyFrontendSlots,
			plugintypes.ResourceOwnerTypeFrontendSlotEntry,
	...
	for _, menu := range manifest.Menus {
		...
		descriptors = append(descriptors, newResourceRefDescriptor(
			plugintypes.ResourceKindMenu,
			strings.TrimSpace(menu.Key),
			plugintypes.ResourceOwnerTypeMenuEntry,
...
func newResourceRefDescriptor(
	kind plugintypes.ResourceKind,
	key string,
	ownerType plugintypes.ResourceOwnerType,
	ownerKey string,
	remark string,
) *plugintypes.ResourceRefDescriptor {
	return &plugintypes.ResourceRefDescriptor{
		Kind:      kind,
		Key:       key,
		OwnerType: ownerType,
		...
		if len(service.Tables) > 0 {
			for _, table := range service.Tables {
				...
				descriptors = append(descriptors, newResourceRefDescriptor(
					kind,
					normalizedTable,
					plugintypes.ResourceOwnerTypeHostServiceResource,
					service.Service,
...
func mapHostServiceResourceKind(service string) plugintypes.ResourceKind {
	switch strings.TrimSpace(service) {
	case protocol.HostServiceStorage:
		return plugintypes.ResourceKindHostStorage
	case protocol.HostServiceNetwork:
		return plugintypes.ResourceKindHostUpstream
	case protocol.HostServiceData:
		return plugintypes.ResourceKindHostData
...
func (s *serviceImpl) BuildResourceRefDescriptors(manifest *catalog.Manifest) []*plugintypes.ResourceRefDescriptor {
