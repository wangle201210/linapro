...
func (s *serviceImpl) buildDynamicRoutePermissionMenuSpecs(manifest *catalog.Manifest) []*catalog.MenuSpec {
	...
	for _, route := range manifest.Routes {
		...
		items = append(items, &catalog.MenuSpec{
			...
			ParentKey: parentKey,
			Name:      plugintypes.DynamicRoutePermissionMenuNamePrefix + permission,
			Perms:     permission,
...
// synthetic route-permission buttons. Prefer a non-button plugin root menu, and
// fall back to the first non-button plugin menu when the plugin mounts under a
// stable host catalog such as the extension center.
...
func buildDynamicRoutePermissionMenuKey(pluginID string, permission string) string {
	...
	return plugintypes.MenuKeyPrefix + strings.TrimSpace(pluginID) + plugintypes.DynamicRoutePermissionMenuKeySeparator + encodedPermission
}
...
func isDynamicRoutePermissionMenuKey(menuKey string) bool {
	return strings.Contains(strings.TrimSpace(menuKey), plugintypes.DynamicRoutePermissionMenuKeySeparator)
}
