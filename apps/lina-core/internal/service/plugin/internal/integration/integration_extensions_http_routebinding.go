// This file captures source-plugin route bindings from GoFrame handlers for the
// integration-owned HTTP registrar without exposing reflection helpers through
// the public pluginhost contract.
...
func captureRouteBindings(
	...
	case reflect.Func:
		return captureFunctionRouteBinding(pluginID, prefix, explicitPattern, explicitMethod, handler)
...
// captureFunctionRouteBinding captures the effective methods and path for one
// function handler, consulting GoFrame request metadata when available.
func captureFunctionRouteBinding(
	pluginID string,
	prefix string,
	explicitPattern string,
	explicitMethod string,
...
// route bindings using GoFrame's conventional method naming rules.
func captureObjectRouteBindings(
	...
	var (
		...
		bindings    = make([]pluginhost.SourceRouteBinding, 0)
	)
	for i := 0; i < reflectValue.NumMethod(); i++ {
		...
		methodBindings := captureFunctionRouteBinding(
			pluginID,
			prefix,
