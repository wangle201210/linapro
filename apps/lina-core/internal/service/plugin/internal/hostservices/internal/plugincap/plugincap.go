// Package plugincap adapts host plugin registry and tenant enablement storage
...
var (
	_ capabilityplugincap.Service      = (*adapter)(nil)
	_ capabilityplugincap.AdminService = (*adapter)(nil)
)
...
func New(
	...
	return &adapter{configFactory: configFactory, state: state, lifecycle: lifecycle}
}

// ForPlugin returns a plugin-bound plugin-domain namespace.
func (a *adapter) ForPlugin(pluginID string) capabilityplugincap.Service {
	if a == nil {
		return nil
	}
	clone := *a
...
func (a *adapter) Config() capabilityplugincap.ConfigService {
...
func (a *adapter) State() capabilityplugincap.StateService {
	if a == nil {
		return nil
	}
	return a.state
...
func (a *adapter) Lifecycle() capabilityplugincap.LifecycleService {
	if a == nil {
		return nil
	}
	return a.lifecycle
...
func (a *adapter) Registry() capabilityplugincap.RegistryService {
	if a == nil {
		return nil
	}
	return a
...
func (a *adapter) BatchGetPlugins(ctx context.Context, _ capmodel.CapabilityContext, ids []capabilityplugincap.PluginID) (*capmodel.BatchResult[*capabilityplugincap.Projection, capabilityplugincap.PluginID], error) {
...
func (a *adapter) ListTenantPlugins(ctx context.Context, capCtx capmodel.CapabilityContext) (*capmodel.PageResult[*capabilityplugincap.TenantProjection], error) {
...
func (a *adapter) SetPluginEnabled(ctx context.Context, capCtx capmodel.CapabilityContext, id capabilityplugincap.PluginID, enabled bool) error {
	tenantID, err := domaincap.TenantID(capCtx.TenantID)
	if err != nil || tenantID <= domaincap.PlatformTenantID {
...
func (a *adapter) ProvisionTenantDefaults(ctx context.Context, _ capmodel.CapabilityContext, tenantIDValue capmodel.DomainID) error {
	tenantID, err := domaincap.TenantID(tenantIDValue)
	if err != nil || tenantID <= domaincap.PlatformTenantID {
		return nil
	}
	rows := make([]*entity.SysPlugin, 0)
...
func (a *adapter) tenantEnabledStates(ctx context.Context, tenantID int, pluginIDs []string) (map[string]bool, error) {
...
func (a *adapter) ensureTenantScopedPlugin(ctx context.Context, pluginID string) error {
...
func (a *adapter) setTenantPluginEnabled(ctx context.Context, tenantID int, pluginID string, enabled bool, insertOnly bool) error {
...
func bumpPluginRuntimeCacheRevision(ctx context.Context, tx gdb.TX) error {
	return domaincap.BumpSharedRevision(
		ctx,
		tx,
		domaincap.PluginRuntimeCacheDomain,
		domaincap.PluginRuntimeCacheScopeGlobal,
		domaincap.TenantPluginRuntimeChangeReason,
	)
}
