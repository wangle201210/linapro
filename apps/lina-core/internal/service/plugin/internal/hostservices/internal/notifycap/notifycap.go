// Package notifycap adapts host notification services and storage to
...
var (
	_ capabilitynotifycap.Service      = (*adapter)(nil)
	_ capabilitynotifycap.AdminService = (*adapter)(nil)
)
...
func (a *adapter) BatchGetMessages(ctx context.Context, capCtx capmodel.CapabilityContext, ids []capabilitynotifycap.MessageID) (*capmodel.BatchResult[map[string]any, capabilitynotifycap.MessageID], error) {
	...
	parsedIDs, requested := domaincap.ParseInt64IDs(ids, func(id capabilitynotifycap.MessageID) {
	...
	tenantID, _ := domaincap.TenantID(capCtx.TenantID)
...
func (a *adapter) Send(ctx context.Context, capCtx capmodel.CapabilityContext, input capabilitynotifycap.SendInput) (*capabilitynotifycap.SendResult, error) {
	...
	if sourceType == notifysvc.SourceTypeNotice && len(input.Recipients) == 0 {
		...
		recipientIDs, parseErr := domaincap.ParsePositiveInt64Strings(input.Recipients)
		if parseErr != nil {
			return nil, bizerr.NewCode(capmodel.CodeCapabilityDenied)
		}
...
func (a *adapter) DeleteMessages(ctx context.Context, capCtx capmodel.CapabilityContext, ids []capabilitynotifycap.MessageID) error {
...
func (a *adapter) DeleteBySource(ctx context.Context, _ capmodel.CapabilityContext, sourceType capabilitynotifycap.SourceType, sourceIDs []string) error {
	if a == nil || a.publisher == nil {
