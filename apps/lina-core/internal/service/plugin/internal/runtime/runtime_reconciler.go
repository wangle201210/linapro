...
func (s *serviceImpl) RefreshInstalledRuntimePluginReleases(ctx context.Context) error {
	...
	var firstErr error
	for _, registry := range registries {
		if err = s.reconcilePrimaryPluginWithLock(ctx, registry, func(ctx context.Context, lockedRegistry *store.PluginRecord) error {
...
func (s *serviceImpl) reconcileDynamicPluginRequest(
	...
	return s.reconcileRuntimePlugin(ctx, pluginID, options)
}
...
func (s *serviceImpl) reconcileRuntimePlugin(
	ctx context.Context,
	pluginID string,
	options DynamicReconcileOptions,
) error {
	...
	return s.reconcileRuntimeRegistryWithOptions(ctx, registry, true, nil, options)
}
...
func (s *serviceImpl) reconcileRuntimeRegistry(
	...
	return s.reconcileRuntimeRegistryWithOptions(ctx, registry, isPrimary, manifestByID, DynamicReconcileOptions{})
}

func (s *serviceImpl) reconcileRuntimeRegistryWithOptions(
	ctx context.Context,
	registry *store.PluginRecord,
	isPrimary bool,
	manifestByID map[string]*catalog.Manifest,
	...
	if isPrimary {
		...
		if err = s.reconcilePrimaryPluginWithLock(ctx, registry, func(ctx context.Context, lockedRegistry *store.PluginRecord) error {
			return s.reconcilePluginIfNeeded(ctx, lockedRegistry, manifestByID, options)
		}); err != nil {
		...
		registry, err = s.storeSvc.GetRegistry(ctx, registry.PluginId)
		if err != nil {
...
func (s *serviceImpl) recoverStaleReconciling(ctx context.Context, registry *store.PluginRecord) (*store.PluginRecord, error) {
	if registry == nil || strings.TrimSpace(registry.CurrentState) != plugintypes.HostStateReconciling.String() {
		return registry, nil
	}
