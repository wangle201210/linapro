...
func (s *serviceImpl) RunDynamicLifecycleCallback(
	...
	if s.wasmRuntime == nil {
		decision.OK = false
		decision.Err = gerror.New("dynamic wasm runtime is not configured")
		decision.Reason = dynamicLifecycleErrorReason(manifest.ID, input.Operation)
		return decision, decision.Err
	}
	response, err := s.wasmRuntime.ExecuteBridge(handlerCtx, wasm.ExecutionInput{
		PluginID:                  manifest.ID,
...
func buildDynamicLifecycleRequest(
	...
	body, err := json.Marshal(bridgecontract.LifecycleRequest{
		...
		FromManifest:     store.PublishedManifestSnapshot(input.FromManifest),
		ToManifest:       store.PublishedManifestSnapshot(input.ToManifest),
	})
	if err != nil {
		return nil, err
	}
