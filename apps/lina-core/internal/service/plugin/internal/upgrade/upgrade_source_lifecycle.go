...
func (s *serviceImpl) buildSourceUpgradePlan(
	currentRelease *store.ReleaseRecord,
	...
	fromSnapshot, err := s.storeSvc.ParseManifestSnapshot(currentRelease.ManifestSnapshot)
	if err != nil {
		return nil, err
	}
	toSnapshot, err := s.storeSvc.ParseManifestSnapshot(targetRelease.ManifestSnapshot)
	if err != nil {
		return nil, err
	}
...
// sourceManifestSnapshotView converts a store snapshot into the stable
// pluginhost manifest snapshot view published to source plugins.
func sourceManifestSnapshotView(snapshot *store.ManifestSnapshot) pluginhost.ManifestSnapshot {
	if snapshot == nil {
		return nil
	}
	return pluginhost.NewManifestSnapshot(store.PublishedManifestSnapshot(snapshot))
}
