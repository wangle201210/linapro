...
func TestExecuteManifestSQLFilesRejectsMockDirection(t *testing.T) {
	...
	err := services.Migration.ExecuteManifestSQLFiles(
		context.Background(),
...
func TestResolveSQLAssetsHandlesMockDirection(t *testing.T) {
	...
	assets, err := services.Migration.ResolveSQLAssets(manifest, plugintypes.MigrationDirectionMock)
	if err != nil {
...
func TestExecuteManifestMockSQLFilesInTxCommitsAllSuccess(t *testing.T) {
	...
	if err = services.Migration.ExecuteManifestSQLFiles(ctx, manifest, plugintypes.MigrationDirectionInstall); err != nil {
		t.Fatalf("expected install SQL to succeed, got error: %v", err)
	}

	var result migration.MockSQLExecutionResult
	txErr := dao.SysPluginMigration.Transaction(ctx, func(txCtx context.Context, _ gdb.TX) error {
		result = services.Migration.ExecuteManifestMockSQLFilesInTx(txCtx, manifest)
		return result.Err
	})
	if txErr != nil {
	...
	rows, err := g.DB().GetAll(ctx, "SELECT migration_key, status FROM sys_plugin_migration WHERE plugin_id = ? AND phase = ? ORDER BY execution_order;", pluginID, plugintypes.MigrationDirectionMock.String())
	if err != nil {
	...
	if rows[0]["status"].String() != plugintypes.MigrationExecutionStatusSucceeded.String() {
...
func TestExecuteManifestMockSQLFilesInTxRollsBackOnFailure(t *testing.T) {
	...
	rows, err := g.DB().GetAll(ctx, "SELECT migration_key FROM sys_plugin_migration WHERE plugin_id = ? AND phase = ?;", pluginID, plugintypes.MigrationDirectionMock.String())
	if err != nil {
		t.Fatalf("expected migration ledger query to succeed, got error: %v", err)
	}
	if len(rows) != 0 {
	...
	installRows, err := g.DB().GetAll(ctx, "SELECT migration_key FROM sys_plugin_migration WHERE plugin_id = ? AND phase = ?;", pluginID, plugintypes.MigrationDirectionInstall.String())
	if err != nil {
...
func TestMockDataLoadErrorUnwrapsCause(t *testing.T) {
	...
	var typed *migration.MockDataLoadError
