// Package manifestresource creates plugin-scoped manifest resource readers for
// source-plugin capability directories and dynamic-plugin WASM host services.
package manifestresource
...
type Factory interface {
	...
	WithArtifactResources(pluginID string, resources map[string][]byte) Factory
}

// EmbeddedFilesResolver returns embedded manifest resources for one source
// plugin. It is injected by the host runtime so manifestresource can stay
// independent from pluginhost.
...
// NewFactory creates a manifest resource service factory.
func NewFactory(developmentRoot string, embeddedResolvers ...EmbeddedFilesResolver) Factory {
...
func (s *serviceAdapter) ForPlugin(pluginID string) manifestcap.Service {
	clone := s.clone()
...
func (s *serviceAdapter) WithArtifactResources(pluginID string, resources map[string][]byte) Factory {
	clone := s.clone()
...
func (s *serviceAdapter) GetMany(ctx context.Context, input manifestcap.GetManyInput) (*manifestcap.GetManyOutput, error) {
	...
	for _, resourcePath := range paths {
		...
		if len(content) > manifestcap.MaxResourceBytes {
			return nil, gerror.Newf("manifest resource exceeds limit path=%s maxBytes=%d", resourcePath, manifestcap.MaxResourceBytes)
		}
		totalBytes += len(content)
		if totalBytes > manifestcap.MaxTotalBytes {
			return nil, gerror.Newf("manifest resources exceed total limit maxBytes=%d", manifestcap.MaxTotalBytes)
		}
...
func (s *serviceAdapter) List(_ context.Context, input manifestcap.ListInput) (*manifestcap.ListOutput, error) {
...
func (s *serviceAdapter) listResources(prefix string, limit int) ([]*manifestcap.Resource, error) {
	resourcesByPath := map[string]*manifestcap.Resource{}
	...
	resources := make([]*manifestcap.Resource, 0, len(paths))
	for _, resourcePath := range paths {
...
func addManifestListResource(resources map[string]*manifestcap.Resource, prefix string, resourcePath string, size int64) {
	normalized, err := normalizeManifestResourcePath(resourcePath)
	if err != nil || (prefix != "" && !strings.HasPrefix(normalized, prefix)) {
		return
	}
	resources[normalized] = &manifestcap.Resource{Path: normalized, Size: size}
}
...
func normalizeManifestResourcePaths(rawPaths []string) ([]string, error) {
	...
	for _, rawPath := range rawPaths {
		...
		if len(paths) > manifestcap.MaxBatchPaths {
			return nil, gerror.Newf("manifest resource path count exceeds limit %d", manifestcap.MaxBatchPaths)
		}
	}
	return paths, nil
}
...
func normalizeManifestListLimit(limit int) int {
	if limit <= 0 {
		return manifestcap.DefaultListLimit
	}
	if limit > manifestcap.MaxListLimit {
		return manifestcap.MaxListLimit
	}
	return limit
}
...
var _ manifestcap.Service = (*serviceAdapter)(nil)
