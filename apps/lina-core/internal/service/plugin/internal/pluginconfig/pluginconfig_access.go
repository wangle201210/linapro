// This file contains the concrete plugin configuration lookup and typed
// conversion operations used by scoped plugin config services.
...
func (s *serviceAdapter) Get(ctx context.Context, key string, defaultValue any) (*gvar.Var, error) {
	...
	var value *gvar.Var
	if defaultValue != nil {
		value, err = cfg.Get(ctx, normalizedKey, defaultValue)
	} else {
	...
	if isMissing(value) && defaultValue != nil {
		return gvar.New(defaultValue), nil
	}
	return value, nil
}
