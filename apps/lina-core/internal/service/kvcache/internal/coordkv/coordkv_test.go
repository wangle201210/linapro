...
func TestCoordKVBackendUsesPlainRedisKeys(t *testing.T) {
	...
	key, err := coordSvc.KeyBuilder().KVKey(
		0,
		kvcache.OwnerTypePlugin.String(),
		"media",
		"route-memory",
...
func TestCoordKVBackendRejectsInvalidTTLAndStringIncrement(t *testing.T) {
	...
	if _, err := service.Set(ctx, kvcache.OwnerTypePlugin, cacheKey, "not-int", time.Minute); err != nil {
		t.Fatalf("seed string value: %v", err)
	}
	if _, err := service.Incr(ctx, kvcache.OwnerTypePlugin, cacheKey, 1, time.Minute); !bizerr.Is(err, kvcache.CodeKVCacheIncrementValueNotInteger) {
...
func TestCoordKVBackendTenantKeysAreIsolated(t *testing.T) {
	...
	if _, err := service.Set(ctx, kvcache.OwnerTypeModule, tenantTwo, "two", time.Minute); err != nil {
		t.Fatalf("set tenant two: %v", err)
	}
	itemOne, ok, err := service.Get(ctx, kvcache.OwnerTypeModule, tenantOne)
	if err != nil || !ok || itemOne.Value != "one" {
		t.Fatalf("expected tenant one value, item=%#v ok=%t err=%v", itemOne, ok, err)
	}
	itemTwo, ok, err := service.Get(ctx, kvcache.OwnerTypeModule, tenantTwo)
