// This file parses and validates public kvcache keys for internal backend
// implementations.
...
const (
	...
	// MaxValueBytes limits string cache payloads for every backend.
	MaxValueBytes = 4096
)

// Identity stores the decoded owner key, namespace, and logical cache key
// parsed from a public cache key string.
...
// TenantID returns the tenant discriminator parsed from the owner key.
func (identity *Identity) TenantID() int64 {
	if identity == nil {
		return 0
	}
...
// OwnerKey returns the decoded owner key segment.
func (identity *Identity) OwnerKey() string {
	if identity == nil {
		return ""
	}
...
// Namespace returns the decoded namespace segment.
func (identity *Identity) Namespace() string {
	if identity == nil {
		return ""
	}
...
// CacheKey returns the decoded logical cache key segment.
func (identity *Identity) CacheKey() string {
	if identity == nil {
		return ""
	}
...
// owner type.
func ResolveIdentity(ownerType OwnerType, cacheKey string) (*Identity, error) {
...
// identity parts.
func parseCacheKey(cacheKey string) (*Identity, error) {
