...
// TestControllerWithTenantScopeReturnsCopy verifies tenant scope does not
// mutate the shared controller used by other cache domains or callers.
func TestControllerWithTenantScopeReturnsCopy(t *testing.T) {
	baseCoord := &fakePluginRuntimeCacheCoordService{markRevision: 15}
	base := NewControllerForScopeWithCoordinator(
		cachecoord.ScopeGlobal,
		RuntimeCacheChangeReason,
		true,
		baseCoord,
	...
	scopedCoord := &fakePluginRuntimeCacheCoordService{markRevision: 16}
	scoped := base.WithTenantScope(31, true)
	scoped.cacheCoordSvc = scopedCoord

	if _, err := scoped.MarkChanged(context.Background()); err != nil {
		t.Fatalf("mark scoped changed failed: %v", err)
	}
	if scopedCoord.markTenantScope.TenantID != 31 || !scopedCoord.markTenantScope.CascadeToTenants {
		t.Fatalf("expected scoped controller tenant metadata, got %#v", scopedCoord.markTenantScope)
	}

	if _, err := base.MarkChanged(context.Background()); err != nil {
		t.Fatalf("mark base changed failed: %v", err)
	}
	if baseCoord.markTenantScope.TenantID != 0 || baseCoord.markTenantScope.CascadeToTenants {
		t.Fatalf("expected base controller to remain unscoped, got %#v", baseCoord.markTenantScope)
	}
}
