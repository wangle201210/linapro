// This file tests typed record store capability enum validation and plan helper behavior.
...
// TestParseRecordStoreEnums verifies all typed record store capability enum parsers accept valid values.
func TestParseRecordStoreEnums(t *testing.T) {
	if _, err := ParsePlanAction("list"); err != nil {
		t.Fatalf("expected valid action, got %v", err)
	}
	if _, err := ParseFilterOperator("eq"); err != nil {
		t.Fatalf("expected valid operator, got %v", err)
	}
	if _, err := ParseOrderDirection("asc"); err != nil {
		t.Fatalf("expected valid direction, got %v", err)
	}
	if _, err := ParseMutationAction("create"); err != nil {
		t.Fatalf("expected valid mutation action, got %v", err)
	}
	if _, err := ParseAccessMode("both"); err != nil {
...
// json.Number so guest-side governed record keys avoid float64 precision loss.
...
func TestQueryPlanJSONRoundTrip(t *testing.T) {
	plan := &QueryPlan{
		...
		Page: &Pagination{PageNum: 1, PageSize: 10},
	}
