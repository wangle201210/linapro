...
// the internal/agents/registry subpackage and offers an interactive
...
func runAgentsSkillsLink(_ context.Context, a *app, input commandInput) error {
	...
	if len(selectors) == 0 {
		results := registry.PlanList(a.root, registry.ResourceSkills)
...
func runAgentsSkillsLinkInteractive(a *app, force bool) error {
	overview := registry.PlanList(a.root, registry.ResourceSkills)
	...
	candidates := registry.LinkCandidates(a.root, registry.ResourceSkills)
...
func executeAgentsSkillsLink(a *app, selectors []string, force bool) error {
	results, err := registry.ApplyLink(a.root, registry.ResourceSkills, registry.LinkRequest{
		Selectors: selectors,
		Force:     force,
	})
	if err != nil {
